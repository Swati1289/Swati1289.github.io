import React from "react";
import "../style.css";

function Skills() {
  return (
    <div id="skill">
      <h1 className="head">SKILLS</h1>

      <ul className="skill-list-1 skill-list">
        <li className="skill-list-item">HTML</li>
        <li className="skill-list-item">CSS</li>
        <li className="skill-list-item">JavaScript</li>
        <li className="skill-list-item">Typescript</li>
        <li className="skill-list-item">React.JS</li>
        <li className="skill-list-item">C++</li>
        <li className="skill-list-item">Java</li>
        <li className="skill-list-item">Python</li>
        <li className="skill-list-item">Spring</li>
        <li className="skill-list-item">Hibernate</li>
        <li className="skill-list-item">Git</li>
        <li className="skill-list-item">SpringMVC</li>
        <li className="skill-list-item">CI/CD</li>
      </ul>
    </div>
  );
}

export default Skills;
